...
class AprilTagDetector{
 ...
 private:
  ...
  tf::Transform myinverse(const tf::Transform tf)
  {
      tf::Matrix3x3 bas=tf.getBasis();
      tf::Matrix3x3 inv=bas.inverse();
      /*
      inv.setValue(bas.getColumn(0).getX(),bas.getColumn(0).getY(),bas.getColumn(0).getZ(),
                           bas.getColumn(1).getX(),bas.getColumn(1).getY(),bas.getColumn(1).getZ(),
                           bas.getColumn(2).getX(),bas.getColumn(2).getY(),bas.getColumn(2).getZ());
                           */
      return tf::Transform(inv, -tf.getOrigin());
  }
 private:
...
private:
  ...
  tf::Transform tag2world,world2tag,camera2base,base2camera;
